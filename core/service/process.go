...
func (this *service) processor() {
	defer func() {
		...
		this.wgStopped.Done()
		this.stop()
		logger2.Logger.Debugf("(%s) Stopping processor", this.cid())
	}()

	logger2.Logger.Debugf("(%s) Starting processor", this.cid())
	...
	for {
		...
		if err != nil {
			if err != io.EOF {
				logger2.Logger.Errorf("(%s) Error peeking next message size", this.cid())
			}
			return
		}
		...
		if err != nil {
			if err != io.EOF {
				logger2.Logger.Errorf("(%s) Error peeking next message: %v", this.cid(), err)
			}
			return
		}

		logger2.Logger.Debugf("(%s) Received: %s", this.cid(), msg)
		...
		if err != nil {
			if err != errDisconnect {
				logger2.Logger.Errorf("(%s) Error processing %s: %v", this.cid(), msg.Name(), err)
			} else {
				return
			}
		}
		...
		if err != nil {
			if err != io.EOF {
				logger2.Logger.Errorf("(%s) Error committing %d read bytes: %v", this.cid(), total, err)
			}
			return
		}
		...
		if this.inStat.msgs%100000 == 0 {
			logger2.Logger.Warn(fmt.Sprintf("(%s) Going to process message %d", this.cid(), this.inStat.msgs))
		}
	}
}
...
func (this *service) processIncoming(msg message.Message) error {
	...
	if err != nil {
		logger2.Logger.Debugf("(%s) Error processing acked message: %v", this.cid(), err)
	}
...
func (this *service) processAcked(ackq *sessions.Ackqueue) {
	for _, ackmsg := range ackq.Acked() {
		...
		if err != nil {
			logger2.Logger.Errorf("process/processAcked: Unable to creating new %s message: %v", ackmsg.Mtype, err)
			continue
		}

		if _, err := msg.Decode(ackmsg.Msgbuf); err != nil {
			logger2.Logger.Errorf("process/processAcked: Unable to decode %s message: %v", ackmsg.Mtype, err)
			continue
		}

		ack, err := ackmsg.State.New()
		if err != nil {
			logger2.Logger.Errorf("process/processAcked: Unable to creating new %s message: %v", ackmsg.State, err)
			continue
		}

		if _, err := ack.Decode(ackmsg.Ackbuf); err != nil {
			logger2.Logger.Errorf("process/processAcked: Unable to decode %s message: %v", ackmsg.State, err)
			continue
		}
		...
		case message.PUBREL:
			...
			if err = this.onPublish(msg.(*message.PublishMessage)); err != nil {
				logger2.Logger.Errorf("(%s) Error processing ack'ed %s message: %v", this.cid(), ackmsg.Mtype, err)
			}

		case message.PUBACK, message.PUBCOMP, message.SUBACK, message.UNSUBACK, message.PINGRESP:
			logger2.Logger.Debugf("process/processAcked: %s", ack)
		...
		default:
			logger2.Logger.Errorf("(%s) Invalid ack message type %s.", this.cid(), ackmsg.State)
			continue
		}
		...
		if ackmsg.OnComplete != nil {
			...
			if !ok {
				logger2.Logger.Errorf("process/processAcked: Error type asserting onComplete function: %v", reflect.TypeOf(ackmsg.OnComplete))
			} else if onComplete != nil {
				if err := onComplete(msg, ack, nil); err != nil {
					logger2.Logger.Errorf("process/processAcked: Error running onComplete(): %v", err)
				}
			}
		}
	}
...
func (this *service) processSubscribe(msg *message.SubscribeMessage) error {
	...
	for i, t := range topics {
		...
		logger2.Logger.Debugf("(%s) topic = %s, retained count = %d", this.cid(), t, len(this.rmsgs))
	...
	logger2.Logger.Infof("客户端：%s，订阅主题：%s，qos：%d，retained count = %d", this.cid(), topics, qos, len(this.rmsgs))
	...
	for _, rm := range this.rmsgs {
		...
		if err := this.publish(rm, nil); err != nil {
			logger2.Logger.Errorf("service/processSubscribe: Error publishing retained message: %v", err)
			rm.SetQoS(old)
			return err
		}
		rm.SetQoS(old)
...
func (this *service) processUnsubscribe(msg *message.UnsubscribeMessage) error {
	...
	logger2.Logger.Infof("客户端：%s 取消订阅主题：%s", this.cid(), topics)
	return err
}
...
func (this *service) onPublish(msg1 *message.PublishMessage) error {
	if msg1.Retain() {
		if err := this.topicsMgr.Retain(msg1); err != nil { // 为这个主题保存最后一条保留消息
			logger2.Logger.Errorf("(%s) Error retaining message: %v", this.cid(), err)
		}
	}
	// todo 集群模式，需要另外设计
	// 发送当前主题的所有共享组
	...
	if err != nil {
		logger2.Logger.Errorf("%v 发送共享：%v 主题错误：%+v", this.id, msg1.Topic(), *msg1)
	}
	// 发送非共享订阅主题
...
func (this *service) pubFn(msg *message.PublishMessage, shareName string, onlyShare bool) error {
	...
	msg.SetRetain(false)
	logger2.Logger.Debugf("(%s) Publishing to topic %s and %s subscribers size ：%v", this.cid(), msg.Topic(), shareName, len(this.subs))
...
func (this *service) pubFnPlus(msg *message.PublishMessage) error {
	...
	msg.SetRetain(false)
	logger2.Logger.Debugf("(%s) Publishing to all shareName topic in %s and %d subscribers：%v", this.cid(), msg.Topic(), len(this.subs))
	for i, s := range this.subs {
		if s != nil {
			fn, ok := s.(*OnPublishFunc)
			if !ok {
