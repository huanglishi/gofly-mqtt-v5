...
import (
	...
	"github.com/huanglishi/gofly-mqttv5/corev5/topicsv5/share"
	"github.com/huanglishi/gofly-mqttv5/corev5/topicsv5/sys"
...
var _ topicsv5.TopicsProvider = (*memTopics)(nil)
...
func memTopicInit() {
	topicsv5.Register("", NewMemProvider())
...
func (this *memTopics) SetStore(_ store.SessionStore, messageStore store.MessageStore) {
...
func (this *memTopics) Retain(msg *messagev5.PublishMessage) error {
	...
	if len(msg.Payload()) == 0 {
		// 删除 DB Retain
		err := this.messageStore.ClearRetainMessage(context.Background(), string(msg.Topic()))
		if err != nil {
			return err
		}
	...
	// 新增 DB Retain
	err := this.messageStore.StoreRetainMessage(context.Background(), string(msg.Topic()), msg)
	if err != nil {
		return err
	}
...
func (this *snode) smatch(topic []byte, qos byte, subs *[]interface{}, qoss *[]topicsv5.Sub) error {
	...
	for k, n := range this.snodes {
		...
		} else if k == topicsv5.SWC || k == level {
			if rem != nil {
...
func nextTopicLevel(topic []byte) ([]byte, []byte, error) {
	...
	for i, c := range topic {
		...
		case '/':
			...
			if i == 0 {
				return []byte(topicsv5.SWC), topic[i+1:], nil
			}
